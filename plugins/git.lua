return {
  require('user.utils').ensure_treesitter_parsers({
    "gitcommit",
    "gitattributes",
    "git_rebase"
  }),
  ...
  -- git goodies for diffs, conflicts and history
  {
    ...
    config = function()
      ...
      local build_keymaps = function(maps)
        local out = {}
        local i = 1
        for lhs, def in pairs(utils.extend_tbl(maps, {
              ["<leader>q"] = { "<cmd>DiffviewClose<cr>", desc = "Quit Diffview" }, -- Toggle the file panel.
